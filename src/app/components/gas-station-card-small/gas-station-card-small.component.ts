...
export class GasStationCardSmallComponent {
  ...
  private readonly gasStationLogos: Record<string, string> = {
    ...
    'rede energia':
      'https://www.redeenergia.pt/wp-content/themes/understrap-child-master/img/og-thumb-2.png',
    'leclerc':
  ...
  /**
   * Check brand name for logo
   */
  private checkMarca(): string | null {
    if (!this.gasStation?.Marca) return null;

    const marca = this.gasStation.Marca.toLowerCase();
    for (const [key, url] of Object.entries(this.gasStationLogos)) {
  ...
  /**
   * Check station name for logo
   */
  private checkNome(): string | null {
    if (!this.gasStation?.Nome) return null;

    const nome = this.gasStation.Nome.toLowerCase();
    for (const [key, url] of Object.entries(this.gasStationLogos)) {
  ...
  getDistance(): string {
    if (
      !this.gasStation?.distance &&
      this.gasStation?.Latitude &&
      this.gasStation?.Longitude
    ) {
      return this.calculateDistance(this.location, [
        this.gasStation.Latitude,
        this.gasStation.Longitude,
      ]);
    }
    return this.gasStation?.distance?.toFixed(2) || '0.00';
  }
  ...
  /**
   * Open maps app with station location
   */
  openMapsApp(event: Event): void {
    event.stopPropagation();

    if (!this.gasStation?.Latitude || !this.gasStation?.Longitude) return;
    if (this.gasStation.Latitude === 0 && this.gasStation.Longitude === 0)
      return;

    const geocoords = `${this.gasStation.Latitude},${this.gasStation.Longitude}`;
    let url: string;

    if (this.platform.is('ios')) {
      url = `maps://?q=${geocoords}`;
    } else if (this.platform.is('android')) {
      url = `geo:${geocoords}?q=${geocoords}`;
    } else {
      url = `https://www.google.com/maps?q=${geocoords}`;
    }
   ...
   */
  onCardClick(): void {
    this.stationClicked.emit(this.gasStation);
  }
  ...
  getPriceValue(): number {
    if (!this.gasStation?.Preco) return 0;
    const cleanPrice = this.gasStation.Preco.replace(/[^\d,]/g, '').replace(
      ',',
      '.'
    );
   ...
   */
  getName(): string {
    return this.gasStation?.Nome || 'Posto sem nome';
  }
   ...
   */
  getAddress(): string {
    if (this.gasStation?.Morada) return this.gasStation.Morada;
    if (this.gasStation?.Localidade) return this.gasStation.Localidade;
    return '';
  }
   ...
   */
  getDistanceColor(): string {
    const distance = parseFloat(this.getDistance());
  ...
  getLastUpdate(): string {
    if (!this.gasStation?.DataAtualizacao) return '';

    const updateDate = new Date(this.gasStation.DataAtualizacao);
    const now = new Date();
